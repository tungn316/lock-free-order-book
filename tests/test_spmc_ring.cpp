...
TEST_CASE("all consumers see every message", "[spmc][single]") {
    ...
    for (int i = 0; i < 10; ++i) { ring.Publish(i); }
...
TEST_CASE("slow consumer is dropped and observes missed count", "[spmc][drop]") {
...
// ── bulk ──────────────────────────────────────────────────────────────────────
// NOTE: This test targets INTENDED behavior. It will FAIL until the bug in
...
TEST_CASE("single producer, many consumers: monotonic, no gaps when keeping up",
    ...
    std::thread producer([&] {
        sync.arrive_and_wait();
        for (int i = 0; i < k_msgs; ++i) { ring.Publish(i); }
    });
