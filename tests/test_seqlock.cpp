...
inline Bbo MakeBbo(
    SeqNum seq, Price bp, Quantity bq, Price ap, Quantity aq)
{
    return {.event_seq = seq, .bid_price = bp, .bid_qty = bq, .ask_price = ap, .ask_qty = aq};
}
    ...
    for (int r = 0; r < k_readers; ++r) {
        readers.emplace_back([&, r] {
            ...
            while (writing.load(std::memory_order::acquire)) {
                const Bbo v = sl.Load();
                if (v.ask_price != v.bid_price + 1) { ++local_torn; }
