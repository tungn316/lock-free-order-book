...
TEST_CASE("push then pop roundtrips FIFO", "[mpsc][single]") {
    MpscRing<int, 8> q;
    for (int i = 0; i < 8; ++i) { CHECK(q.TryPush(i)); }
    for (int i = 0; i < 8; ++i) {
        auto v = q.TryPop();
        REQUIRE(v.has_value());
        CHECK(*v == i);
...
TEST_CASE("push fails when full", "[mpsc][single]") {
    MpscRing<int, 4> q;
    for (int i = 0; i < 4; ++i) { CHECK(q.TryPush(i)); }
...
TEST_CASE("many producers, single consumer: no loss, no dup", "[mpsc][concurrent]") {
    ...
    std::thread consumer([&] {
        ...
        while (consumed < k_total) {
            ...
            if (seq != next_expected[pid]) { ordering_ok = false; }
            ++next_expected[pid];
            ++consumed;
        }
    });
