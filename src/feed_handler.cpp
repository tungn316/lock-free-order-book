...
namespace lfob {
    ...
    bool FeedHandler::Join() noexcept
    {
        ...
        } else if (::inet_pton(AF_INET, m_config.iface.data(),
    ...
    std::size_t FeedHandler::Receive() noexcept
    {
        ...
        const int n{::recvmmsg(m_fd, msgs.data(),
                               static_cast<unsigned>(k_datagram_batch), 0,
                               nullptr)};
        if (n <= 0) {
    ...
    std::uint64_t FeedHandler::Gaps() const noexcept { return m_mold.Gaps(); }

    bool FeedHandler::Healthy() const noexcept { return m_fd >= 0 && !m_stalled; }
