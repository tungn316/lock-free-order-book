...
namespace lfob {
    ...
    bool ClientSession::PublishPublic(const std::byte* frame, std::size_t bytes) noexcept
    {
    ...
    bool ClientSession::PublishPrivate(const std::byte* frame, std::size_t bytes) noexcept
    {
    ...
    std::size_t ClientSession::Flush() noexcept { return m_out.FlushTo(m_fd); }

    [[nodiscard]] bool ClientSession::NeedsResync() const noexcept { return (m_state.load(std::memory_order::relaxed) == State::GAPPED); }

    void ClientSession::Close() noexcept { m_state = State::CLOSED; }

    bool ClientSession::SendGapNotice(SeqNum resume_seq) noexcept
    {
        const ExecutionReport notice{.seq=resume_seq, .type=ExecutionReport::Type::GAP_NOTICE, .client=m_id};
    ...
    {
        const ExecutionReport snap{.seq=bbo.event_seq, .type=ExecutionReport::Type::TOP_OF_BOOK, .client=m_id, .bid_price=bbo.bid_price, .bid_quantity=bbo.bid_quantity, .ask_price=bbo.ask_price, .ask_quantity=bbo.ask_quantity};
        ...
        m_state.compare_exchange_strong(expected, State::ACTIVE, std::memory_order::relaxed);
    ...
    void ClientSession::MarkGapped() noexcept
    {
        ...
        m_state.compare_exchange_strong(expected, State::GAPPED, std::memory_order::relaxed);
    }

    [[nodiscard]] std::uint64_t ClientSession::Dropped() const noexcept { return m_dropped; }
    [[nodiscard]] ClientId ClientSession::Id() const noexcept { return m_id; }
    [[nodiscard]] int ClientSession::Fd() const noexcept { return m_fd; }
    [[nodiscard]] ClientSession::State ClientSession::GetState() const noexcept { return m_state.load(std::memory_order::relaxed); }
    ...
    ClientSession::FillResult ClientSession::FillInbound() noexcept
    {
        ...
        const ssize_t r{::read(m_fd, m_in.data() + m_in_tail, m_in.size() - m_in_tail)};
        if (r > 0) {
