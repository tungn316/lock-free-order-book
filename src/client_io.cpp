...
namespace lfob {
    ...
    bool ClientIo::Listen(std::uint16_t port) noexcept
    {
        ...
        if (::bind(m_listen_fd, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr)) < 0) {
            ::close(m_listen_fd);
            m_listen_fd = -1;
            return false;
        }

        if (::listen(m_listen_fd, SOMAXCONN) < 0) {  // start queuing incoming connections
            ::close(m_listen_fd);
            m_listen_fd = -1;
            return false;
        }
    ...
    void ClientIo::Start()
    {
        assert(m_listen_fd >= 0 && m_epoll_fd >= 0 && "Listen() must precede Start()");
    ...
    void ClientIo::OnAcceptable() noexcept
    {
        ...
        while (true) {
            ...
            const int cfd{::accept4(m_listen_fd,
            ...
            std::unique_ptr<ClientSession> session(
                new (std::nothrow) ClientSession(id, cfd));
            if (!session) {
                ::close(cfd);
                m_rejected.fetch_add(1, std::memory_order::relaxed);
                continue;
            ...
            if (::epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, cfd, &ev) < 0) {
                m_rejected.fetch_add(1, std::memory_order::relaxed);
                continue;  // session unwinds -> ~ClientSession closes cfd (removes it from epoll)
            }
