  ...
  void Publish(const T& item) noexcept
{
    ...
    for (auto i{0UZ}; i < count; ++i) {
        ...
        if (m_producer_pos - cur >= CAPACITY) {
            m_cursors[i].pos.store(m_producer_pos - CAPACITY + 1, std::memory_order::release);
            m_cursors[i].missed.fetch_add(m_producer_pos - cur - CAPACITY + 1, std::memory_order::relaxed);
        }
    }

    Slot& slot = m_buffer[m_producer_pos & (CAPACITY - 1)];
    slot.payload = item;
    slot.ready.store(static_cast<SeqNum>(m_producer_pos + 1), std::memory_order::release);
...
{
    return (m_tail.load(std::memory_order::relaxed) - m_cursors[id].pos.load(std::memory_order::relaxed));
}
